...
- **Summaries collapse the messages they cover** *(on)* — once a scene is
  ...
  out of them, and a marker is posted in chat at the end of the range covered.
...
- **Rolling world state, smart context, memory lifecycle and post-turn processor
  ...
  exposed, so they could not be turned on or tuned without editing code.
...
- **A batch of eight fixes** across `tool_choice` double-wrapping, slash commands,
  ...
  when it should have been passed through.
...
- **The static-entry guard is honoured on lifecycle and post-turn writes,** which
  had bypassed it and could rewrite constant entries.
- **Remember deduplicates on meaning rather than character overlap,** and the
  sidecar accepts an empty API key for local endpoints that don't need one.
- **The floating button renders correctly on mobile viewports.**
